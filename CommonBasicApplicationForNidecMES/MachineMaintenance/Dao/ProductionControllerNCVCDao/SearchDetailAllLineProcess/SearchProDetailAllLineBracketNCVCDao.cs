...
{
    public class SearchProDetailAllLineBracketNCVCDao : AbstractDataAccessObject
    {
        public override ValueObject Execute(TransactionContext trxContext, ValueObject vo)
        {
            ...
            sql.Append("times between '00:00:00' and '05:59:00' then dates+1 end datesss, model_cd,'All Line' line_cd, sum(ba_app_metal_deform_scracth) ba_app_metal_deform_scracth, ");
            sql.Append("sum(ba_app_ba_deform) ba_app_ba_deform, sum(ba_app_endplate_deform_scracth) ba_app_endplate_deform_scracth ,");
            sql.Append("sum(ba_app_error_other) ba_app_error_other from ");

            sql.Append("(select i2.dates,i2.times,i2.model_cd,i2.line_cd, ba_app_metal_deform_scracth, ba_app_ba_deform, ba_app_endplate_deform_scracth, ba_app_error_other from t_ncvp_pdc_ba i2 left join (select dates, line_cd, Case when idca3 is null then idca1 else idca3 end id  from(select tblca1.dates, tblca1.line_cd, idca1, idca3  from(select line_cd, o.dates, max(o.ba_id) idca1  from t_ncvp_pdc_ba o where o.times > '06:00:00' and o.times <= '23:59:00' and o.dates >= :datefrom and o.dates <= :dateto group by o.dates, line_cd order by dates) tblca1 left join(select line_cd, (o.dates - 1) dates, max(o.ba_id) idca3  from t_ncvp_pdc_ba o  where o.times > '00:00:00' and o.times <= '05:30:00' and o.dates > :datefrom and o.dates - 1 <= :dateto group by line_cd, o.dates order by idca3) tblca3 on tblca1.dates = tblca3.dates and tblca1.line_cd = tblca3.line_cd) tbl  order by dates, line_cd) l on l.line_cd = i2.line_cd  where i2.ba_id = l.id order by i2.dates,i2.line_cd ) t where model_cd = :model_cd  group by datesss,model_cd order by datesss");
