...
{
    class TotalRankDEPWareHouseMainDao : AbstractDataAccessObject
    {
...
select tam.rank_name as Account_Name,sum(tam.acquistion_cost) as Total_AcquisitionCose,sum(tam.current_depreciation) as Total_CurrentDepreication,sum(tam.monthly_depreciation) as Total_MonthlyDepreication,sum(tam.accum_depreciation_now) 
...
e.asset_cd, e.asset_no, e.asset_name, e.asset_model, e.asset_serial, e.asset_supplier, g.qty, a.account_code_cd, b.account_location_cd,
 f.rank_cd, b.account_location_name, g.comment_data, e.asset_life,f.rank_name, e.acquistion_date, e.acquistion_cost, g.depreciation_start, g.depreciation_end,
 ...
 from ( select tam.rank_name as Account_Name,sum(tam.acquistion_cost) as acquistion_cost,sum(tam.current_depreciation) as current_depreciation,sum(tam.monthly_depreciation) as monthly_depreciation,sum(tam.accum_depreciation_now) 
as accum_depreciation_now,sum(tam.net_value) as net_value from (select a.account_code_name, g.warehouse_main_id,
e.asset_cd, e.asset_no, e.asset_name, e.asset_model, e.asset_serial, e.asset_supplier, g.qty, a.account_code_cd, b.account_location_cd,
 f.rank_cd, b.account_location_name, g.comment_data, e.asset_life,f.rank_name, e.acquistion_date, e.acquistion_cost, g.depreciation_start, g.depreciation_end,
 g.current_depreciation,g.monthly_depreciation, g.accum_depreciation_now, g.net_value, e.asset_invoice, g.registration_date_time, g.registration_user_cd from t_warehouse_main g
                           left join m_account_code a on a.account_code_id = g.account_code_id
                           left join m_account_location b on b.account_location_id = g.account_location_id
                            ...
                            left join m_user_location d on d.user_location_id = g.user_location_id
                            left join m_asset e on e.asset_id = g.asset_id
                            left join m_rank f on f.rank_id = g.rank_id) tam group by tam.rank_name) tam) tbltotal order by account_name = 'Total'
