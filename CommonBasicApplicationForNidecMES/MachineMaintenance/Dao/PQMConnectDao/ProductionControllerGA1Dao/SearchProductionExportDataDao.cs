...
namespace Com.Nidec.Mes.Common.Basic.MachineMaintenance.Dao
{
    class SearchProductionExportDataDao : AbstractDataAccessObject
    {
        public override ValueObject Execute(TransactionContext trxContext, ValueObject vo)
        {
            ...
            sql.Append("select a.process, b.inspect, sum(b.inspectdata) from ldp_5sg201902 a left join ldp_5sg201902data b on a.serno = b.serno where a.inspectdate >= :datefrom and a.inspectdate <= :dateto and a.model = :model and line =:line and a.process not in ('FA_IP','GC_IP') group by a.process, b.inspect");
            sql.Append(" union ");
            sql.Append("select 'FA_IP' process, 'Input Frame' inspect, sum(b.inspectdata) from ldp_5sg201902 a left join ldp_5sg201902data b on a.serno = b.serno where a.inspectdate >= :datefrom and a.inspectdate <= :dateto and a.process in ('FA_IP') and a.model = :model and line =:line");
            sql.Append(" union ");
            sql.Append("select 'GC_IP' process, 'Input Gear' inspect, sum(b.inspectdata) from ldp_5sg201902 a left join ldp_5sg201902data b on a.serno = b.serno where a.inspectdate >= :datefrom and a.inspectdate <= :dateto and a.process in ('GC_IP') and a.model = :model and line =:line");
            sql.Append(" union ");
            ...
            sql.Append(" union ");
            sql.Append("Select 'TOTAL_M' process, 'Toal NG Motor' inspect, sum(b.inspectdata) from ldp_5sg201902 a left join ldp_5sg201902data b on a.serno = b.serno  where a.inspectdate >= :datefrom and a.inspectdate <= :dateto and a.process in ('MC_FPC','MC_FWCHK','MC_Mark','MC_NOICHK','MC_STMASS')  and a.model = :model and line =:line");
            sql.Append(" union ");
